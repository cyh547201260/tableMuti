...
function provEvent () {
    $('#_citys0 a').click(function () {
        ...
        if ($('#' + ths.dataset.prefix + '_prov').length == 0) {
            var hcitys = $('<input>', {
                type: 'hidden',
                name: ths.dataset.prov ? ths.dataset.prov : 'prov',
                'data-name': lev,
...
function cityEvent () {
    $("#_citys1 a").click(function () {
        ...
        if ($('#' + ths.dataset.prefix + '_city').length == 0) {
            var hcitys = $('<input>', {
                type: 'hidden',
                name: ths.dataset.city ? ths.dataset.city : 'city',
                'data-name': lev,
        ...
        //批量加人表格扩展
        $(ths).trigger('onchange');
        console.log(cell)
        if(cell){
        	setAddressCallback(cell)
