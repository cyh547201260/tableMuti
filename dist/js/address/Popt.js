...
//      }
    ...
    autoCheck: function (def, e) {
        if (def != '') {
            var data = def.split(this.confg.separator);
            var provObj = document.getElementById('_citys0').querySelector('a[data-name=' + data[0] + ']');
            document.getElementById('_citys1').innerHTML = this.getCity(provObj);
            this.$addClass(provObj, 'AreaS');
            if (data.length > 1) {
                // 选中的是省和市
                var cityObj = document.getElementById('_citys1').querySelector('a[data-name=' + data[1] + ']');
                this.$addClass(cityObj, 'AreaS');
            }
            e();
        }
    },
    getProv: function () {
        ...
        for (var i = 0, len = b.length; i < len; i++) {
            prov.push('<a data-level="0" data-id="' + b[i]['id'] + '" data-name="' + b[i]['name'] + '">' + b[i]['name'] + '</a>');
        }
        return prov.join('');
    },
    getCity: function (obj) {
        var c = obj.dataset.id;
        var e = province;
        var f;
        var g = '';
